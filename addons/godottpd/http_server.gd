...
var server_identifier: String = "API"

# stores partial request data per client
var _buffers := {}
...
func _process(_delta: float) -> void:
	if _server:
		...
		if new_client:
			...
			_buffers[new_client] = ""  # init buffer
		for client in _clients:
			if client.get_status() == StreamPeerTCP.STATUS_CONNECTED:
				var bytes = client.get_available_bytes()
				if bytes > 0:
					var chunk = client.get_string(bytes)
					_buffers[client] += chunk
					_try_parse_request(client)

func _try_parse_request(client: StreamPeer) -> void:
	var buff = _buffers[client]
	# check if headers are complete
	var header_end = buff.find("\r\n\r\n")
	if header_end == -1:
		return # headers not done yet
	var header_text = buff.substr(0, header_end)
	var body_start = header_end + 4
	var content_length = 0
	for line in header_text.split("\r\n"):
		if line.begins_with("Content-Length:"):
			content_length = int(line.split(":")[1].strip_edges())
	# check if full body is here
	if buff.length() < body_start + content_length:
		return # body incomplete
	# full request received
	var full_request = buff.substr(0, body_start + content_length)
	# clear buffer for next request
	_buffers[client] = buff.substr(body_start + content_length)
	_handle_request(client, full_request)
...
func _perform_current_request(client: StreamPeer, request: HttpRequest):
	_print_debug("New request to %s"%request.path)
	var found = false
	var response = HttpResponse.new()
	response.client = client
	response.server_identifier = server_identifier
	for router in self._routers:
#		if request.method == "GET" and request.path.begins_with("/images/"):
#			if router.router is ImagesRouter:
#				found = true
